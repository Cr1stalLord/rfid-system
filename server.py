...
.input-group input { padding: 10px; border-radius: 8px; border: 1px solid #555; background: #222; color: white; min-width: 120px; flex: 1; }
.btn { background: #e94560; border: none; color: white; padding: 10px 25px; border-radius: 8px; cursor: pointer; font-size: 1em; transition: 0.3s; }
...
.reg-block { margin-top: 20px; padding: 20px; background: rgba(255,255,255,0.03); border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); }
    ...
    <div>
      ...
      <div class="card" style="margin-top:20px;">
        ...
        <div style="max-height:300px;overflow-y:auto;">
          <table><thead><tr><th>Имя</th><th>UID</th><th>Статус</th><th>Действие</th></tr></thead><tbody id="usersList"></tbody></table>
        </div>
      </div>
    </div>
  </div>
...
async function deleteUser(uid) {
  ...
  await fetch(API_URL + '/delete-user?uid=' + uid, { method: 'DELETE' });
  loadData();
}
...
async function loadData() {
  ...
  if (data.users && data.users.length > 0) {
    data.users.forEach(u => {
      ...
      usersList.innerHTML += `<tr><td>${u.name} ${u.surname}</td><td>${u.uid}</td><td style="color:${color}">${status}</td><td><button class="btn btn-danger" onclick="deleteUser('${u.uid}')">🗑️</button></td></tr>`;
    });
  } else {
...
def handle_rfid():
    ...
    if not user:
        ...
        c.execute("INSERT INTO users (uid, name, surname, is_inside, exit_count) VALUES (?, 'User', 'User', 0, 0)", (uid,))
        c.execute("REPLACE INTO card_status (uid, is_inside) VALUES (?, 0)", (uid,))
        c.execute("INSERT INTO events (action, uid, timestamp) VALUES ('register', ?, ?)", (uid, now))
        conn.commit()
